...
float Compute2dPerlinNoise( float posX, float posY, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotSouthWest = Vector2::DotProduct( gradientSW, displacementFromSW );
		float dotSouthEast = Vector2::DotProduct( gradientSE, displacementFromSE );
		float dotNorthWest = Vector2::DotProduct( gradientNW, displacementFromNW );
		float dotNorthEast = Vector2::DotProduct( gradientNE, displacementFromNE );
...
float Compute3dPerlinNoise( float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotBelowSW = Vector3::DotProduct( gradientBelowSW, displacementFromBelowSW );
		float dotBelowSE = Vector3::DotProduct( gradientBelowSE, displacementFromBelowSE );
		float dotBelowNW = Vector3::DotProduct( gradientBelowNW, displacementFromBelowNW );
		float dotBelowNE = Vector3::DotProduct( gradientBelowNE, displacementFromBelowNE );
		float dotAboveSW = Vector3::DotProduct( gradientAboveSW, displacementFromAboveSW );
		float dotAboveSE = Vector3::DotProduct( gradientAboveSE, displacementFromAboveSE );
		float dotAboveNW = Vector3::DotProduct( gradientAboveNW, displacementFromAboveNW );
		float dotAboveNE = Vector3::DotProduct( gradientAboveNE, displacementFromAboveNE );
...
float Compute4dPerlinNoise( float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotBeforeBSW = Vector4::DotProduct( gradientBeforeBSW, displacementFromBeforeBSW );
		float dotBeforeBSE = Vector4::DotProduct( gradientBeforeBSE, displacementFromBeforeBSE );
		float dotBeforeBNW = Vector4::DotProduct( gradientBeforeBNW, displacementFromBeforeBNW );
		float dotBeforeBNE = Vector4::DotProduct( gradientBeforeBNE, displacementFromBeforeBNE );
		float dotBeforeASW = Vector4::DotProduct( gradientBeforeASW, displacementFromBeforeASW );
		float dotBeforeASE = Vector4::DotProduct( gradientBeforeASE, displacementFromBeforeASE );
		float dotBeforeANW = Vector4::DotProduct( gradientBeforeANW, displacementFromBeforeANW );
		float dotBeforeANE = Vector4::DotProduct( gradientBeforeANE, displacementFromBeforeANE );
		float dotAfterBSW = Vector4::DotProduct( gradientAfterBSW, displacementFromAfterBSW );
		float dotAfterBSE = Vector4::DotProduct( gradientAfterBSE, displacementFromAfterBSE );
		float dotAfterBNW = Vector4::DotProduct( gradientAfterBNW, displacementFromAfterBNW );
		float dotAfterBNE = Vector4::DotProduct( gradientAfterBNE, displacementFromAfterBNE );
		float dotAfterASW = Vector4::DotProduct( gradientAfterASW, displacementFromAfterASW );
		float dotAfterASE = Vector4::DotProduct( gradientAfterASE, displacementFromAfterASE );
		float dotAfterANW = Vector4::DotProduct( gradientAfterANW, displacementFromAfterANW );
		float dotAfterANE = Vector4::DotProduct( gradientAfterANE, displacementFromAfterANE );
