...
void Render()
{
	...
	Vector2 bottomLeft = Vector2(0.f, 0.f);
	Vector2 topRight = Vector2(1000.f, 1000.f);
	g_theRenderer->SetOrtho( bottomLeft, topRight );

	// Clear all screen (backbuffer) pixels to medium-blue
	Rgba blackColor = Rgba(0, 0, 0, 255);
	g_theRenderer->ClearScreen(blackColor);

	// Begine drawing a line
	g_theApp->Render();
...
void RunFrame()
{
	g_theApp->BeginFrame();		// [ ENGINE ]
	g_theApp->Update();			// [ GAME ]
	Render();					// [ GAME ] #SD1ToDo: ...becomes with g_theApp->Render();
	g_theApp->EndFrame();		// [ ENGINE ]
}
