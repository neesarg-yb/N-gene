	...
	: GameState( "PROTOSCENE 3D", parentClock )
{
	...
	CameraBehaviour* followBehaviour	= new CB_Follow( 6.3f, 40.f, 70.f, 100.f/*, 1.f, 179.f*/, "Follow", m_cameraManager );
...
void Scene_ProtoScene3D::JustFinishedTransition()
{
	...
	ConsolePrintf( RGBA_GREEN_COLOR, "Scene_ProtoScene3D: Press [T] to toggle the debug camera" );
}
...
RaycastResult Scene_ProtoScene3D::Raycast( Vector3 const &startPosition, Vector3 const &direction, float maxDistance )
{
	PROFILE_SCOPE_FUNCTION();
...
Vector3 Scene_ProtoScene3D::SphereCollision( Sphere const &sphere )
{
	PROFILE_SCOPE_FUNCTION();
...
void Scene_ProtoScene3D::ChangeClocksTimeScale()
{
...
void Scene_ProtoScene3D::PauseUnpauseClock()
{
	if( g_theInput->WasKeyJustPressed( 'P' ) )
		m_clock->IsPaused() ? m_clock->Resume() : m_clock->Pause();
}

void Scene_ProtoScene3D::ChangeDebugCameraSettings()
{
...
void Scene_ProtoScene3D::DebugRenderHotkeys()
{
	// Pause the Game
